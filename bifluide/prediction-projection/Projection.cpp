...
void Projection::ComputeProjectionStep(Solution& solNext,
                                       Solution& sol,PiercedVector<double>& rho1,PiercedVector<double>& rhoFC)
{
  ...
  for (auto& inter: _grid->getInterfaces())
  {
    ...
    kappaFC[inter.getId()] = 1./rhoFC[inter.getId()];
  }
                                         ...
                                         Solution& sol,PiercedVector<double>& rho1,PiercedVector<double>& rhoFC)
{
  ...
  solNext.VelocityFC[interId] -= solNext.dt/rhoFC[interId] * gradFC[interId];
