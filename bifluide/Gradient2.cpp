...
UserDataComm<double>* Gradient2::_userComm;
int Gradient2::_userComm_only_one_init_dirty=0;

Gradient2::Gradient2(int dim, Grid *grid, StencilBuilder* stencil)
  : _grid(grid),
  _stencil(stencil),
...
std::vector<std::pair<long,double> > Gradient2::buildVertexStencil(const long ownerId, const int localVertex) {
...
std::vector<std::pair<long,double> > Gradient2::computeNormalGradient(const long interfaceId, std::vector<std::vector<std::pair<long,double> > > &grad){
...
std::map<std::pair<int,int>,double> Gradient2::buildSizeMap(const long interfaceId){
...
std::vector<std::vector<std::pair<long,double> > > Gradient2::buildInterfaceGradientStencil(const long interfaceId){
...
std::vector<std::pair<long,double> > Gradient2::buildInterfaceNormalGradientStencil(const long interfaceId){
...
std::vector<std::array<double,3> >  Gradient2::computeFVGradient(PiercedVector<double> &val)
{
...
std::vector<std::array<double,3> > Gradient2::computeLSGradient(PiercedVector<double> &val){
  PiercedVector<std::array<double,3> > gradient;

  if (Gradient2::_userComm_only_one_init_dirty!=10101)
  {
    Gradient2::_userComm = new UserDataComm<double>(*_grid, 1);
    Gradient2::_userComm_only_one_init_dirty=10101;
  }

  Gradient2::_userComm->update(val.size());
  Gradient2::_userComm->communicate(val);
...
std::vector<std::array<double,3> > Gradient2::computeLSGradient(const std::vector<double> &val){

  PiercedVector<double> a = VtoPV(val,_grid);
  return computeLSGradient(a);
}

std::array<double,3> Gradient2::computeLSCellGradient(long id, PiercedVector<double> &data){
...
void Gradient2::buildLSCellGridMatrix(long id, std::vector<long> &neigh,double* A){
...
void Gradient2::buildLSCellRHS(long id, std::vector<long> &neigh,PiercedVector<double> &data, double *F){
...
PiercedVector<std::array<double, 3> > Gradient2::computeCCLSGradient(
...
std::vector<PiercedVector<std::array<double, 3> > > Gradient2::computeCCLSGradient(
  ...
  const std::vector<double>& times)
{
  ...
  {
    Gradient2::_userComm->update();
    Gradient2::_userComm->communicate(val[i]);
  }
  ...
  for (size_t i=0; i<nbVal; i++)
  {
    Gradient2::_userComm->update(3);
    Gradient2::_userComm->communicate(gradients[i]);
  }
  return gradients;
}

Stencil Gradient2::buildCellGradientStencil(const long& cellId)
{
  bool isUniform(true);
...
PiercedVector<double> Gradient2::computeFCLSGradient(
  PiercedVector<double>& val,
  const double t,
  const Var type)
{
...
std::vector<PiercedVector<double> > Gradient2::computeFCLSGradient(
  std::vector<PiercedVector<double> >& val,
  const std::vector<Var>& types,
  const std::vector<double>& times)
{
  ...
  ASSERT(types.size() == val.size(),
    ...
    vector of values in Gradient2::computeFCLSGradient method. You have to \
...
Stencil Gradient2::buildFaceNormalGradientStencil(const long& interId)
{
...
PiercedVector<std::array<double, 3> > Gradient2::computeFVGradient(
  PiercedVector<double>&  val,
  const double t,
  const Var type)
{
...
PiercedVector<double> Gradient2::computeFVDiffusion(
  PiercedVector<double>&  val, PiercedVector<double>&  muFC)
{
  ...
  for (std::size_t i=0; i<3; ++i) {
    ...
    else
    {
      ...
      if (!gradient.exists(owners[0]))
        gradient.emplace(owners[0]);
      NPoint ghostOctCenter =  _grid->evalCellCentroid(owners[0]);
      ghostOctCenter += pow(-1,face+1)
                             * _grid->evalCellSize(owners[0]);

  std::array<double,3> centerVec, c1 = ghostOctCenter, c0 = _grid->evalCellCentroid(owners[0]);
  double d=0;
  for (std::size_t i=0; i<3; ++i) {
    centerVec[i] = c1[i]-c0[i];
    d += (c1[i]-c0[i])*(c1[i]-c0[i]);
      ...
      double value=0;
      value =0;
      gradient[owners[0]] += value * muFC[interId]*area/d;
