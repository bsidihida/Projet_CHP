...
double taylorGreen_Ux(NPoint pt,double t)
{
  return sin(pt[0])* cos(pt[1]) * F(t);
}

double taylorGreen_Uy(NPoint pt,double t)
{
...
int main(int ac, char **av)
{
  ...
  Solution sol, solPrev, solNext,solexacte;
  ...
  Grid     *grid = new Grid(0,0,0, 2*M_PI, 0.02, &BC, GRID_2D);
  ...
  for (auto &cell: grid->getCells())
  {
    ...
    sol.Ux[cellId] = taylorGreen_Ux(grid->evalCellCentroid(cellId),0.);
    sol.Uy[cellId] = taylorGreen_Uy(grid->evalCellCentroid(cellId),0.);
  ...
  for (auto &cell: grid->getCells())
  {
    ...
    solexacte.Ux.emplace(cellId);
    solexacte.Uy.emplace(cellId);

    solexacte.Ux[cellId] = taylorGreen_Ux(grid->evalCellCentroid(cellId),-100.);
    solexacte.Uy[cellId] = taylorGreen_Uy(grid->evalCellCentroid(cellId),-100.);

    solexacte.VelocityCC.emplace(cellId);
    solexacte.VelocityCC[cellId] = {solexacte.Ux[cellId],solexacte.Uy[cellId],0.};

    solexacte.Pressure.emplace(cellId);
    solexacte.Pressure[cellId] = 0.;
  ...
  // --------------sol_exacte--------
  auto velCC_exacte = PVtoV(solexacte.VelocityCC, grid);
  grid->setExportName("TaylorGreen_exact");
  grid->addData("Vel_exacte", velCC_exacte);
  grid->write();
