...
int main(int ac, char **av) {
  ...
  if (ac != 3) {
    std::cerr << av[0] << " arg1: 1 for ASphere, 2 for STL\n arg2: lambdacoeff (double) " << std::endl;
    exit(1);
  }

  int typeGeom = std::stoi(av[TYPEGEOM]);
  double lambdaCoeff = std::stod(av[LAMBDACOEFF]);
  ...
  std::vector<double> lambda = ls->getLevelSet("TAG");

  if (lambdaCoeff != 0) {

    // simple constant value init
    //for (auto & element : lambda) {
    //  element = lambdaCoeff;
    // }
    ...
    for (auto &cell : g->getCells())
      {
    	...
    	if (g->getCell(id).isInterior()) {
	      ...
	      lambda[count]=  lambdaCoeff*sin(3.141592653589*y); //*sin(3.141592653589*x);
	      }
        count++;
      }
    lap->setLambda(&lambda);
  }
  ...
  g->setExportName("lambda-laplacian");
