...
double He2(double phi, double epsilon) {
    if (phi < -epsilon) {
        return 0.0;
...
double rho2(double phi,double epsilon,double rho1,double rho2)
{
  return rho1+(rho2-rho1)*He2(phi,epsilon);
}



double mu2(double phi,double epsilon,double mu1,double mu2)
{
  return mu1+(mu2-mu1)*He2(phi,epsilon);
  
}
// double rho1(NPoint pt,double rho1,double rho2)
// {
//   if(pt[1]>3)
//   return rho1;
//   else{
...
// double mu1(NPoint pt,double mu1,double mu2)
// {
//   if(pt[1]>3)
//   return mu1;
//   else{
...
double rhof1=1000.,rhof2=1.,epsilon2=2*0.1;
...
void Projection::ComputeProjectionStep(Solution& solNext,
                                       Solution& sol,PiercedVector<double> phiFC, PiercedVector<double> phiCC)
{
  ...
  for (auto& cell: _grid->getCells())
  {
     ...
     _rhocc[cell.getId()]=rho2(phiCC[cell.getId()], epsilon2, rhof1, rhof2);
    }
 for (auto& inter:_grid->getInterfaces())
  {

  
    _rhofc[inter.getId()]=rho2(phiFC[inter.getId()], epsilon2, rhof1, rhof2);
  }
  ...
  for (auto& inter: _grid->getInterfaces())
  {
    ...
    kappaFC[inter.getId()] = 1./_rhofc[inter.getId()];
  }
 ...
 for (auto& cell: _grid->getCells()){
      ...
      const long& id = cell.getId();
       ...
       solNext.Pressure[id] = sol.Pressure[id]
                     + _psi[id]/solNext.dt;
  ...
  // Compute correction
  this->computeCorrection(solNext,phiFC,phiCC);
}
                                         ...
                                         Solution& sol,PiercedVector<double> phiFC, PiercedVector<double> phiCC)
{
 ...
 for (auto& inter:_grid->getInterfaces())
  ...
  solNext.VelocityFC[interId] -= solNext.dt/_rhofc[interId]* gradFC[interId];
...
void Projection::computeCorrection(Solution& solNext,PiercedVector<double> phiFC, PiercedVector<double> phiCC)
...

{
 ...
 for (auto& inter:_grid->getInterfaces())
  ...
  solNext.VelocityFC[inter.getId()] -= gradFC[inter.getId()]/_rhofc[inter.getId()];
  ...
  {
  const long& cellId = cell.getId();
  solNext.VelocityCC[cellId] -= gradCC[cellId]/_rhocc[cellId];
